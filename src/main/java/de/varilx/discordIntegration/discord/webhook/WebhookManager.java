...
public class WebhookManager implements DiscordHandler {
    ...
    public void manageLifecyle(String type) {
        if (!configuration.getString("chatbridge.type").equalsIgnoreCase("webhook")) return;
    ...
    private void create(ErrorConsumer<DiscordWebhook> consumer) {
        if (this.configuration.getString("chatbridge.webhook.url") == null) return;

        CompletableFuture.runAsync(() -> {
            try {
                DiscordWebhook webhook = new DiscordWebhook(this.configuration.getString("chatbridge.webhook.url"));
